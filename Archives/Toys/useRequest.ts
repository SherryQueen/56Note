...
const createRequestHook = (globalOption: IRequestHookOption) => {
  const useRequest = <R = unknown, Q extends object = object>(
    ...
    const _option = useRef<IRequestHookOption<R, Q>>({ ...globalOption, ...option.option })
    ...
    useEffect(() => {
      ...
      setState((prevState) => ({ ...prevState, loading: true, status: IRequestStatus.LOADING }))
      ...
      Promise.race([_request.current.call(null, param), p])
        .then((data) => {
          setState((prevState) => ({ ...prevState, loading: false, data, status: IRequestStatus.SUCCESS }))
          if (onSuccess) onSuccess(data, param)
        })
        .catch((err) => {
          setState((prevState) => ({ ...prevState, loading: false, status: IRequestStatus.FAILURE }))
      ...
      return () => {
        abort()
        setState((prevState) => ({ ...prevState, loading: false, status: IRequestStatus.CANCEL }))
      }
...
export const useRequest = createRequestHook({
  errorPrompt: (err) => notification.error({ message: '请求失败', description: `请求失败原因: ${err.message}` }),
})
