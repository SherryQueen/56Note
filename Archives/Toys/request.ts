...
export const handleResult: IHandleResult = (data) => {
  ...
  if ("code" in data || "status_code" in data) {
    ...
    if (code !== 0 && code !== "0")
      throw new Error(getErrorMessage(data) || "未知错误");
    return data.data;
  }
  return data;
};
...
export class IRequest implements IRequestOption {
      ...
      fetch(_url, opt)
        .catch((err) => {
          if (err.name === "TypeError")
            throw new Error(`当前网络异常, 请稍后重试: ${err.message}`);
  ...
  request<T = any>(
    url: string,
    option: IRequestAPIOption & { preHandle?: boolean }
  ): Promise<T> {
    const promise = this._request(url, option);
    return option?.preHandle ?? true
      ? promise.then(handleResponse).then(this._handleResult)
      : promise;
  }
