...
export class AuthError extends Error {
  redirect: string
  constructor(redirect: string, msg?: string) {
    super(`Auth Error: ${msg || 'unAuth'}`)
...
export const getURLWithSearch = (url: string, query?: Record<string, any>): string => {
  ...
  return url.includes('?') ? (url.endsWith('&') ? search : `&${search}`) : `?${search}`
}
...
export const handleResult = (
  ...
  if ('code' in data || 'status_code' in data) {
    ...
    if (code === 302 || code === '302') throw new AuthError(data.data)
...
export class IRequest implements IRequestOption {
  ...
  request<T = any>(url: string, option: IRequestAPIOption & { preHandle?: boolean }): Promise<T> {
    ...
    return option?.preHandle ?? true ? promise.then(handleResponse).then(handleResult) : promise
  }
